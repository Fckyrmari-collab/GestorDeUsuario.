...
def editar_empleado(id):
    ...
    return render_template("editarempleado.html", empleado=empleado, areas=lista_areas)
...
def actualizar_empleado():
    ...
    sql = """
    UPDATE empleados 
    SET documento=%s, nombre=%s, apellido=%s, cargo=%s, salario=%s,
        horas_extras=%s, bonificacion=%s, salud=%s, pension=%s, salario_neto=%s, id_area=%s
    WHERE id_empleado=%s
    """

    cursor.execute(sql, (documento, nombre, apellido, cargo, salario,
                         horas, bonificacion, salud, pension, salario_neto, area, id))
    con.commit()
...
def panel_empleado ():
    ...
    cursor.execute("""
        ...
        INNER JOIN usuarios u ON u.documento = e.documento
        WHERE u.usuario = %s
    """, (session['usuario'],))
